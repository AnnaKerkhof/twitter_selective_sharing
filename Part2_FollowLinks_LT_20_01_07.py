...
# - 15.01.2020: Anpassung an zusätzliche Spalte Bundesland für LT
...
with open('Results LT\Part 1\LT_OnlyURL_2019_07_15_P3.csv', newline='', encoding='utf-8') as csvfile:
	...
	for row in namesreader:
		...
		if expanded_url == "None":
			...
			if parsed_utf.netloc == "twitter.com":
				...
				if len(row) > 11:
					...
					link_source = row[9]
					quoted_handle = row[10]
					quoted_tweet_id = row[11]
					quoted_tweet_text = row[12]

					zwischenliste.append(name)
					zwischenliste.append(handle)
					zwischenliste.append(party)
					zwischenliste.append(bundesland)
		...
		if row_counter == 1000:
			...
			with open("Results LT\Part 2\LinksUnshortened_2020_01_04_LT_P3.csv", "w", newline="", encoding="utf-8") as ergebnisCSV:
