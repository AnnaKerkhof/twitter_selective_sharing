...
ergebnisse = []
counter = 0
BuTa_constitution = datetime.strptime("2018-12-28 09:00", "%Y-%m-%d %H:%M")		# angepasst an 199 Tage Zeitraum der ursprünglichen Daten
...
def Datenabfrage(name, handle, party, landtag, BuTa_constitution, ergebnisse, latest_id, max_counter, tweet_counter, overview, all_tweets):
	...
	try:
		...
		for tweet in twitterabfrage:
			...
			tweetdaten = []
			all_tweets.append([name, handle, party, landtag, tweet_created, tweet._json['id'], tweet._json['full_text']])
	...
	if tweet_created != None and user_account_created != None and latest_id != None and max_counter > 0 and statuses_count > 0:
		...
		if tweet_created > BuTa_constitution and tweet_created > user_account_created and max_counter < 3200 and max_counter < statuses_count:
			...
			Datenabfrage(name = name, handle = handle, party = party, landtag = landtag, BuTa_constitution = BuTa_constitution, ergebnisse = ergebnisse, latest_id = latest_id, max_counter = max_counter, tweet_counter = tweet_counter, overview = overview, all_tweets = all_tweets)

		elif max_counter >= 3200:
			error_list.append([name, handle, party, landtag, "Reached Tweet Limit! Last tweet at: " + str(tweet._json['user']['created_at']), tweet_counter, latest_id])
			overview.append([name, handle, party, landtag, tweet_counter])
			return ergebnisse
		...
		overview.append([name, handle, party, landtag, tweet_counter, "FAULT"])
		error_list.append([name, handle, party, landtag, "Fault other than Tweepy", tweet_counter])
		print(""
...
with open("AccountHandles\\NamesLT.csv", newline="", encoding="utf-8") as csvfile:
	...
	for row in namesreader:
		...
		if row[1] != 'None':
			...
			Datenabfrage(name = name, handle = handle, party = party, landtag = landtag, BuTa_constitution = BuTa_constitution, ergebnisse = ergebnisse, latest_id = None, max_counter = 0, tweet_counter = 0, overview = overview, all_tweets = all_tweets)
...
with open("Results LT\\Part 1\\LT_OnlyURL_2019_07_15.csv", "w", newline="", encoding="utf-8") as ergebnisCSV:
...
with open("Results LT\\Part 1\\LT_Overview_2019_07_15.csv", "w", newline="", encoding="utf-8") as overviewCSV:
...
with open("Results LT\\Part 1\\LT_All_Tweets_2019_07_15.csv", "w", newline="", encoding="utf-8") as AllCSV:
...
with open("Results LT\\Part 1\\LT_Errors_2019_07_15.csv", "w", newline="", encoding="utf-8") as ErrorsCSV:
